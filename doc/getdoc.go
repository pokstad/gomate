...
func (p *Position) UnmarshalJSON(data []byte) error {
	...
	if len(m) != 4 {
		return errors.New("unexpected number of regexp matches for position: " + strconv.Itoa(len(m)))
	}

	line, err := strconv.Atoi(m[2])
	if err != nil {
		return errors.Wrap(err, "unable to parse line number: "+err.Error())
	}

	index, err := strconv.Atoi(m[3])
	if err != nil {
		return errors.Wrap(err, "unable to parse index number: "+err.Error())
	}
...
func (g Getter) LookupSymbol(ctx context.Context, c gomate.Cursor) (Symbol, error) {
	...
	if err != nil {
		return Symbol{}, errors.Wrap(err, "can't determine offset")
	}
	...
	if err != nil {
		...
		return Symbol{}, errors.Wrap(err, "unable to run gogetdoc")
	}
	...
	if err := json.NewDecoder(stdout).Decode(&d); err != nil {
		return Symbol{}, errors.Wrap(err, "unable to decode gogetdoc JSON output")
	}
